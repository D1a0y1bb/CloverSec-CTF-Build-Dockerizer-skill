...
`v2.2.0-r4` 是 `v2.2.0` 的第四个发布修复版本，不改变 `challenge.yaml` 配置契约和核心单题渲染行为。重点处理 r3 使用反馈里剩下的两个边界：`solve_probe` 需要正式 example 覆盖，`flag.sync_paths` 需要明确它是在 `/changeflag.sh` 阶段同步业务路径。

本次 r4 修复覆盖：

- `python-flask-basic` 正式带上 `challenge.verification.solve_probe`，现在内置 examples 里有可直接执行的业务入口断言样例。
- `smoke_test.sh --case python-flask-basic` 会从 `challenge.yaml` 读取 HTTP probe，验证容器启动后返回 `hello from python-flask-basic`。
- `flag.sync_paths` 的说明更加明确：它由生成的 `/changeflag.sh` 同步到业务路径，不代表任意 `start.sh` 启动时都会自动写好 `flag0/flag1` 等文件。
- Pwn/Web 真实题仍要求看源码确认业务 flag 路径；如果题目不读 `/flag`，需要配置 `flag.sync_paths`，并用 `solve_probe`、`smoke_assert.yaml` 或实际解题验证入口。

r3 已经修复 Pwn/BOF 误判、PHP 7.4 保留、`accept --refresh`、`--pretty`、业务 flag 路径和 `solve_probe` 主链路问题；r4 的目标是把 `solve_probe` 变成正式样例覆盖，并把动态 flag 同步边界写清楚，避免使用者误以为工具能自动推断所有题目业务路径。
