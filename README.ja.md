...
`v2.2.0-r9` は `v2.2.0` の 9 回目のリリース修正版です。`challenge.yaml` contract と core script behavior は変更しません。standalone GitHub skill、plugin-bundled skill、local installed skill の entry instructions を同期します。

この r9 release の主な修正点：

- `SKILL.md` と `agents/openai.yaml` は plugin-bundled Dockerizer behavior を基準にし、`workflow.py auto-render` を推奨します。
- 通常の challenge は platform delivery files を生成し static validation を実行します。unknown container runtime、privileged execution、不明な business decision は user confirmation で停止します。
- Java JAR sample の `build/` files を standalone repository に入れ、standalone release package と plugin-bundled skill の内容を一致させます。
...
`v2.2.0-r7` は `v2.2.0` の 7 回目のリリース修正版です。`challenge.yaml` contract と通常の単一問題レンダリング挙動は変更しません。公式 Skill validation との互換性、入力ヒントの配置、Docker artifact の archive field、サンプル大容量ファイル管理を扱います。

この r7 release の主な修正点：

- 公式 Skill validation に合わせ、非標準のトップレベル Skill field を削除しました。`challenge.yaml` と `--project-dir` の入力ヒントは `SKILL.md` と `agents/openai.yaml` に移しています。
- Docker artifact workflow は引き続き `environment`、`docker_artifacts`、`xlsx_fields` を出力し、amd64 image tar archive と spreadsheet handoff に使えます。
- Linux-QEMU sample の `initrd.img` を Git 管理対象から外し、`.gitignore` で大容量生成ファイルの再混入を防ぎます。
